...
export const cloudDemoDashboardSummaryIos = async (ctx) => {
  ...
  try {
    const metadata = await ctx.appambit.db(
      ...
      ['table', 'cloud_demo_tasks_ios', 'cloud_demo_orders_ios'],
    );
    databaseAvailable = true;
    const tableNames = new Set(metadata.results[0].rows.map((row) => row[0]));
    databaseTablesReady = tableNames.has('cloud_demo_tasks_ios')
      && tableNames.has('cloud_demo_orders_ios');
